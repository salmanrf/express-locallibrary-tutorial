...
exports.genre_create_post = [
    ...
    body('name', 'Genre name requried').trim().isLength({min: 1}).escape(),
...
exports.genre_delete_get = function(req, res) {
    async.parallel({
        genre: (callback) => Genre.findById(req.params.id, callback),
        books: (callback) => Book.find({'genre': req.params.id}, callback)
    }, (err, {genre, books}) => {
        if(err) return next(err);
        if(!genre) res.redirect('/catalog/books');

        res.render('genre_delete', {title: 'Delete Book', genre, books});
    });
};
...
exports.genre_delete_post = function(req, res) {
    async.parallel({
        genre: (callback) => Genre.findById(req.body.genreid, callback),
        books: (callback) => Book.find({'genre': req.body.genreid}, callback)
    }, (err, {genre, books}) => {
        if(err) return next(err);
...
exports.genre_update_post = [
    ...
    (req, res, next) => {
        ...
        Genre.findByIdAndUpdate(req.body.id, genre, {}, (err, genre) => {
            if(err) return next(err);
            res.redirect(genre.url);
        });
    }
